  ...
  <div class="min-h-screen bg-[var(--color-background)] py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
          <h1 class="text-3xl font-bold text-[var(--color-heading)]">Authors</h1>
          <p class="mt-2 text-[var(--color-text)] opacity-75">Browse authors in the database</p>
        </div>
        <div v-if="authStore.isAuthenticated" class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none flex gap-2">
          <!-- Regular users can create authors -->
          <button @click="navigateToAddAuthor"
      ...
      <div v-if="loading" class="text-center py-12">
        <div class="inline-flex items-center">
          ...
          <span class="text-xl text-[var(--color-text)]">Loading authors...</span>
        </div>
      </div>
      ...
      <!-- Empty State -->
      <div v-else-if="displayedAuthors.length === 0 && !loading" class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-[var(--color-text)] opacity-75">{{ searchQuery ? 'No authors found' : 'No authors' }}</h3>
        <p class="mt-1 text-sm text-[var(--color-text)] opacity-50">
          {{ searchQuery ? 'Try adjusting your search terms.' : 'Get started by creating a new author.' }}
        </p>
      ...
      <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="author in displayedAuthors"
             :key="author.id"
          ...
          <div class="p-6">
            <div class="flex items-center justify-between">
              <div class="flex-1 min-w-0">
                <h3 class="text-lg font-medium text-[var(--color-heading)] truncate">{{ author.name }}</h3>
                <p v-if="author.otherName" class="text-sm text-[var(--color-text)] opacity-60 truncate mt-1">
                  Also known as: {{ author.otherName }}
                </p>
              </div>
            </div>

            <div v-if="author.description" class="mt-3">
              <p class="text-sm text-[var(--color-text)] opacity-75 line-clamp-3">{{ author.description }}</p>
            </div>

            <div class="mt-4 flex items-center justify-between text-xs text-[var(--color-text)] opacity-50">
              <span>{{ author.titleCount }} title{{ author.titleCount !== 1 ? 's' : '' }}</span>
