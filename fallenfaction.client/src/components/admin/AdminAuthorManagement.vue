<!-- components/admin/AdminAuthorManagement.vue -->
<template>
  <div class="min-h-screen bg-[var(--color-background)] py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Page Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-[var(--color-heading)]">Author Management</h1>
        <p class="mt-2 text-[var(--color-text)] opacity-75">Add, edit, and manage authors</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div class="lg:col-span-2">
          <div class="bg-[var(--color-background-soft)] rounded-lg shadow-md border border-[var(--color-border)]">
            <div class="px-6 py-4 border-b border-[var(--color-border)]">
              <h3 class="text-lg font-semibold text-[var(--color-heading)]">Author Management</h3>
            </div>
            ...
            <div class="p-6">
              <form @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <!-- Author Name -->
                <div>
                  <label for="authorName" class="block text-sm font-medium text-[var(--color-text)] mb-2">
                ...
                <!-- Other Name -->
                <div>
                  <label for="authorOtherName" class="block text-sm font-medium text-[var(--color-text)] mb-2">
                    Other Name
                  </label>
                  <input id="authorOtherName"
                ...
                <!-- Description -->
                <div>
                  <label for="authorDescription" class="block text-sm font-medium text-[var(--color-text)] mb-2">
                    Description
                  </label>
                  <textarea id="authorDescription"
        ...
        <!-- Right Column: Author List -->
        <div class="lg:col-span-3">
          <div class="bg-[var(--color-background-soft)] rounded-lg shadow-md border border-[var(--color-border)]">
            <div class="px-6 py-4 border-b border-[var(--color-border)]">
              <h3 class="text-lg font-semibold text-[var(--color-heading)] mb-4">Author List</h3>
              <input type="text"
                     v-model="searchQuery"
                     @input="handleSearch"
            ...
            <div v-else class="overflow-x-auto max-h-96">
              <table class="min-w-full divide-y divide-[var(--color-border)]">
                ...
                <tbody class="bg-[var(--color-background-soft)] divide-y divide-[var(--color-border)]">
                  <tr v-for="author in displayedAuthors"
                      :key="author.id"
                      class="hover:bg-[var(--color-background-mute)] transition-colors duration-200"
                      :class="{ 'bg-green-50 border-l-4 border-l-green-500': selectedAuthorId === author.id }">
              ...
              <!-- No Results -->
              <div v-if="displayedAuthors.length === 0 && !isLoadingAuthors" class="text-center py-8">
                ...
                <h3 class="mt-2 text-sm font-medium text-[var(--color-text)]">No authors found</h3>
                <p class="mt-1 text-sm text-[var(--color-text)] opacity-75">{{ searchQuery ? 'Try adjusting your search terms' : 'Start by adding a new author' }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    setup() {
      ...
      const loadAuthors = async () => {
        ...
        try {
          ...
          showMessage('Failed to load authors', 'error');
        } finally {
      ...
      const handleSearch = () => {
        ...
        searchTimeout = setTimeout(async () => {
          ...
          try {
            const result = await authorService.searchAuthors(searchQuery.value.trim());
            if (result.success) {
      ...
      const searchAsYouType = () => {
        // This mimics the original behavior where typing in name field searches authors
        if (form.name.trim()) {
      ...
      const editAuthor = (author) => {
        ...
        form.otherName = author.otherName || '';
      ...
      const updateAuthor = async () => {
        ...
        try {
          ...
          const result = await authorService.updateAuthor(form.id, authorData);
      ...
      const deleteAuthor = async () => {
        ...
        try {
          const result = await authorService.deleteAuthor(form.id);
          ...
          showMessage('Failed to delete author', 'error');
        } finally {
          loading.value = false;
        }
      };
