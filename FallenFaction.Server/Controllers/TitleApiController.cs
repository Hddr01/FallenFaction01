            ...
            try
            {
                ...
                if (existingTitle.Description != request.Description)
                {
                    changeLogs.Add(new TitleChangeLog
                    {
                        ...
                        OldValue = existingTitle.Description ?? "",
                ...
                if (isTrustedForEditTitle)
                {
                    ...
                    if (editTitle != null)
                    {
                        // EnableRetryOnFailure does not retry inside explicit transactions;
                        // wrapping in ExecuteAsync gives us both atomicity and SQL resilience.
                        ...
                        await strategy.ExecuteAsync(async () =>
                        {
                            ...
                            try
                            {
                                foreach (var cl in changeLogs)
                                {
                                    ...
                                    await _changeApplicator.ApplyAsync(editTitle, cl, HttpContext.RequestAborted);
